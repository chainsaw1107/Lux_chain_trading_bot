...
const balance = async () => {
  ...
  console.log(
    "Main wallet Balance is : ",
    ethers.utils.formatEther(mBalance),
    "\n\n"
  );
  for (let i = 0; i < transactionCount; i++) {
    ...
    const tokenbalance_wallet = await tokenContract.balanceOf(
      walletArray[i].wallet.address
    );
    console.log(
      `Wallet${i + 1}'s ether Balance: ${ethers.utils.formatEther(
        EthBalance_wallet
      )}`
    );
    console.log(
      `Wallet${i + 1}'s token Balance: ${ethers.utils.formatUnits(
        tokenbalance_wallet
      )}\n`
    );
  }
